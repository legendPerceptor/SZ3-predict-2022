...
//    size_t cur_file_index = 0;
    ...
    for(size_t i=world_rank;i<num_of_files;i+=world_size) {
        ...
        int start_location= sizeof(MPI_UNSIGNED) * (world_size + 2);
        for(size_t j=0;j < i % world_size;j++){
        ...
        printf("My rank is %d, dealing with No. %zu file, writing offset: %d, compressed_size: %zu, compression time: %lf, compression ratio: %lf\n",
               world_rank, round, start_location, compressed_size, cp_result.CPTime, cp_result.CR);
